...
select_result * Select(customer * cus,orders * ord,lineitem * item,char * order_date,char * ship_date,char * mktsegment)//进行选择
{
                ...
                if (cus[i].c_custkey==ord[j].o_custkey && strcmp(ord[j].o_orderdate, order_date)<0){
                    for (k=0; k<1000; k++){
                        if (ord[j].o_orderkey==item[k].l_orderkey && strcmp(item[k].l_shipdate, ship_date)>0){
                            result[sindex].l_extendedprice = item[k].l_extendedprice;
                            result[sindex].l_orderkey = item[k].l_orderkey;
                            strcpy(result[sindex].o_orderdate, ord[j].o_orderdate);
                            sindex++;
                        }
                    }
                }
    ...
    for (i=0; i<sindex-1; i++){
        for (j=0; j<sindex-1; j++){
            if (result[j].l_extendedprice < result[j+1].l_extendedprice){
                tmp = result[j+1].l_extendedprice;
                result[j+1].l_extendedprice =result[j].l_extendedprice;
                result[j].l_extendedprice = tmp;
            }
        }
    }
